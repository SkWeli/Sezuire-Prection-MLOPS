...
def train(
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        threshold_result = find_best_threshold(
            ...
            window_step_s=window_step_s,
            smoothing_window=0,
            selection_policy=alarm_threshold_policy,
            min_specificity=min_alarm_specificity,
        )
        ...
        print(
            ...
            f"using policy={alarm_threshold_policy}"
        )
        print(
            "       validation alarm metrics: "
            f"balanced_acc={best_val_alarm_metrics['alarm_balanced_accuracy']:.4f}, "
            f"sensitivity={best_val_alarm_metrics['alarm_recall_sensitivity']:.4f}, "
            f"specificity={best_val_alarm_metrics['alarm_specificity']:.4f}, "
            f"F1={best_val_alarm_metrics['alarm_f1']:.4f}, "
            f"predicted_alarm_rate={best_val_alarm_metrics['alarm_prediction_rate']:.4f}"
        )
        if (
            alarm_threshold_policy == "specificity_constrained"
            and not threshold_result["specificity_constraint_met"]
        ):
            print(
        ...
        print(f"  Selection policy         : {alarm_threshold_policy}")
        ...
        print(f"  Alarm Youden J           : {test_metrics['alarm_youden_j']:.4f}")
        print(f"  Predicted alarm rate     : {test_metrics['alarm_prediction_rate']:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["balanced_accuracy", "youden_j", "f1", "specificity_constrained"],
        default="balanced_accuracy",
        help=(
            "Validation policy for the secondary alarm threshold. "
            "balanced_accuracy is the recommended default."
        ),
    )
    parser.add_argument(
        "--min-alarm-specificity",
        type=float,
        default=0.80,
        help=(
            "Minimum validation specificity when using "
    ...
    sys.exit(
        train(
            ...
            alarm_threshold_policy=args.alarm_threshold_policy,
            min_alarm_specificity=args.min_alarm_specificity,
        )
    )
